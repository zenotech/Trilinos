...
#ifndef Tempus_StepperBE_impl_hpp
#define Tempus_StepperBE_impl_hpp

#include "Tempus_config.hpp"
#include "Tempus_StepperFactory.hpp"
#include "Tempus_StepperBEModifierDefault.hpp"
...
StepperBE<Scalar>::StepperBE()
{
  this->setStepperType(        "BE");
...

{
  ...
  this->setBEAppAction(stepperBEAppAction);
...
template<class Scalar>
void StepperBE<Scalar>::setPredictor(std::string predictorType)
{
...
/// Set the predictor.
template<class Scalar>
void StepperBE<Scalar>::setPredictor(
...
template<class Scalar>
void StepperBE<Scalar>::setBEAppAction(
  Teuchos::RCP<StepperBEAppAction<Scalar> > appAction)
{
  if (appAction == Teuchos::null) {
    // Create default appAction
    stepperBEAppAction_ = Teuchos::rcp(new StepperBEModifierDefault<Scalar>());
  } else {
...
template<class Scalar>
void StepperBE<Scalar>::initialize()
{
  TEUCHOS_TEST_FOR_EXCEPTION(
    this->wrapperModel_ == Teuchos::null, std::logic_error,
    "Error - Need to set the model, setModel(), before calling "
    "StepperBE::initialize()\n");
}


template<class Scalar>
void StepperBE<Scalar>::setInitialConditions(
  const Teuchos::RCP<SolutionHistory<Scalar> >& solutionHistory)
{
  ...
  if (this->getUseFSAL()) {
    ...
    Teuchos::OSTab ostab(out,1,"StepperBE::setInitialConditions()");
...
template<class Scalar>
void StepperBE<Scalar>::takeStep(
  const Teuchos::RCP<SolutionHistory<Scalar> >& solutionHistory)
{
  using Teuchos::RCP;

  TEMPUS_FUNC_TIME_MONITOR("Tempus::StepperBE::takeStep()");
  {
    TEUCHOS_TEST_FOR_EXCEPTION(solutionHistory->getNumStates() < 2,
      std::logic_error,
      "Error - StepperBE<Scalar>::takeStep(...)\n"
      "Need at least two SolutionStates for Backward Euler.\n"
      "  Number of States = " << solutionHistory->getNumStates() << "\n"
      "Try setting in \"Solution History\" \"Storage Type\" = \"Undo\"\n"
      "  or \"Storage Type\" = \"Static\" and \"Storage Limit\" = \"2\"\n");

    RCP<StepperBE<Scalar> > thisStepper = Teuchos::rcpFromRef(*this);
    stepperBEAppAction_->execute(solutionHistory, thisStepper,
      StepperBEAppAction<Scalar>::ACTION_LOCATION::BEGIN_STEP);
    ...
    Teuchos::RCP<TimeDerivative<Scalar> > timeDer =
      Teuchos::rcp(new StepperBETimeDerivative<Scalar>(
        Scalar(1.0)/dt,xOld));
    ...
    stepperBEAppAction_->execute(solutionHistory, thisStepper,
      StepperBEAppAction<Scalar>::ACTION_LOCATION::BEFORE_SOLVE);

    const Thyra::SolveStatus<Scalar> sStatus =
      this->solveImplicitODE(x, xDot, time, p);

    stepperBEAppAction_->execute(solutionHistory, thisStepper,
      StepperBEAppAction<Scalar>::ACTION_LOCATION::AFTER_SOLVE);
    ...
    stepperBEAppAction_->execute(solutionHistory, thisStepper,
      StepperBEAppAction<Scalar>::ACTION_LOCATION::END_STEP);
  }
  return;
}

template<class Scalar>
void StepperBE<Scalar>::computePredictor(
      const Teuchos::RCP<SolutionHistory<Scalar> >& solutionHistory)
{
  ...
  if (solutionHistory->getWorkingState()->getSolutionStatus()==Status::FAILED) {
    ...
    Teuchos::OSTab ostab(out,1,"StepperBE::computePredictor");
...
template<class Scalar>
void StepperBE<Scalar>::describe(
...
StepperBE<Scalar>::getValidParameters() const
{
...
template <class Scalar>
int
StepperBE<Scalar>::stencilLength() const
{
  return 2;
}
...
template <class Scalar>
void
StepperBE<Scalar>::computeStepResidual(
...
template <class Scalar>
void
StepperBE<Scalar>::computeStepJacobian(
...
template <class Scalar>
void
StepperBE<Scalar>::computeStepParamDeriv(
...
template <class Scalar>
void
StepperBE<Scalar>::computeStepSolver(
...
template <class Scalar>
void
StepperBE<Scalar>::computeStepResidDerivImpl(
  ...
  const int deriv_index) const
{
  ...
  Teuchos::RCP<TimeDerivative<Scalar> > timeDer =
    Teuchos::rcp(new StepperBETimeDerivative<Scalar>(Scalar(1.0)/dt,xo));
...
#endif // Tempus_StepperBE_impl_hpp
