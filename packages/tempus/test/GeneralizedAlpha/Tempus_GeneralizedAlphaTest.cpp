...
// ************************************************************
// ************************************************************
TEUCHOS_UNIT_TEST(GeneralizedAlpha, ConstructingFromDefaults)
{
  double dt = 1.0;

  // Read params from .xml file
  RCP<ParameterList> pList = getParametersFromXmlFile(
    "Tempus_GeneralizedAlpha_HarmonicOscillator_Damped_SecondOrder.xml");
  ...
  auto stepper =
    Teuchos::rcp(new Tempus::StepperGeneralizedAlpha<double>(model));
  ...
  auto solutionHistory = Teuchos::rcp(new Tempus::SolutionHistory<double>());
...
//// ************************************************************
//TEUCHOS_UNIT_TEST(GeneralizedAlpha, HarmonicOscillatorDamped_SecondOrder)
//{
//  RCP<Tempus::IntegratorBasic<double> > integrator;
...
//      writeSolution("Tempus_GeneralizedAlpha.dat", solutionHistory);
//
//      auto solnHistExact =Teuchos::rcp(new Tempus::SolutionHistory<double>());
//      for (int i=0; i<solutionHistory->getNumStates(); i++) {
//        double time = (*solutionHistory)[i]->getTime();
//        auto state = Teuchos::rcp(new Tempus::SolutionState<double>(
//            model->getExactSolution(time).get_x(),
