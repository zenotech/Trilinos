...
// ************************************************************
// ************************************************************
TEUCHOS_UNIT_TEST(BE, ParameterList)
{
...
// ************************************************************
// ************************************************************
TEUCHOS_UNIT_TEST(BE, ConstructingFromDefaults)
{
  double dt = 0.1;
  ...
  auto stepper = rcp(new Tempus::StepperBE<double>());
...
TEUCHOS_UNIT_TEST(BE, SinCos)
{
  ...
  for (int n=0; n<nTimeStepSizes; n++) {
    ...
    if (n == 0) {
      ...
      writeSolution("Tempus_BE_SinCos.dat", solutionHistory);
      ...
      writeSolution("Tempus_BE_SinCos-Ref.dat", solnHistExact);
    }
  ...
  writeOrderError("Tempus_BE_SinCos-Error.dat",
                  stepper, StepSize,
...
TEUCHOS_UNIT_TEST(BE, CDR)
{
  ...
  for (int n=0; n<nTimeStepSizes; n++) {
    ...
    if ((n == nTimeStepSizes-1) && (comm->NumProc() == 1)) {
      std::ofstream ftmp("Tempus_BE_CDR.dat");
  ...
  double xDotSlope = 0.0;
  RCP<Tempus::Stepper<double> > stepper = integrator->getStepper();
  writeOrderError("Tempus_BE_CDR-Error.dat",
                  stepper, StepSize,
                  solutions,    xErrorNorm,    xSlope,
                  solutionsDot, xDotErrorNorm, xDotSlope);
  ...
  if (comm->NumProc() == 1) {
    ...
    std::ofstream ftmp("Tempus_BE_CDR-Solution.dat");
...
// ************************************************************
// ************************************************************
TEUCHOS_UNIT_TEST(BE, VanDerPol)
{
  RCP<Tempus::IntegratorBasic<double> > integrator;
  std::vector<RCP<Thyra::VectorBase<double>>> solutions;
  std::vector<RCP<Thyra::VectorBase<double>>> solutionsDot;
  ...
  for (int n=0; n<nTimeStepSizes; n++) {
    ...
    if ((n == 0) or (n == nTimeStepSizes-1)) {
      std::string fname = "Tempus_BE_VanDerPol-Ref.dat";
      if (n == 0) fname = "Tempus_BE_VanDerPol.dat";
      RCP<const SolutionHistory<double> > solutionHistory =
        integrator->getSolutionHistory();
  ...
  RCP<Tempus::Stepper<double> > stepper = integrator->getStepper();
  double order = stepper->getOrder();
  writeOrderError("Tempus_BE_VanDerPol-Error.dat",
                  stepper, StepSize,
                  solutions,    xErrorNorm,    xSlope,
                  solutionsDot, xDotErrorNorm, xDotSlope);
...
// ************************************************************
// ************************************************************
TEUCHOS_UNIT_TEST(BE, OptInterface)
{
  // Read params from .xml file
  RCP<ParameterList> pList =
    getParametersFromXmlFile("Tempus_BackwardEuler_SinCos.xml");
